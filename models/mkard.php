...
class Mkard {
    ...
    // Listar todos los periodos de la empresa para selectores en inventario
    public function getPeriodosKardex() {
        try {
            if (!$this->getIdemp()) return [];
            $sql = "SELECT idkar, anio, mes, cerrado,
                           CONCAT(anio,'/',LPAD(mes,2,'0')) AS periodo,
                           CASE WHEN cerrado=1 THEN 'Cerrado' ELSE 'Abierto' END AS estado
                    FROM kardex WHERE idemp = :idemp ORDER BY anio DESC, mes DESC";
            $modelo = new conexion();
            $conexion = $modelo->get_conexion();
            ...
            error_log("Error en Mkard::getPeriodosKardex() - " . $e->getMessage());
            return [];
        }
    }
}
